...
namespace CESCO.IoT.DeviceManagement
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            deviceMgr.RegisterDevice("CESCO003").Wait();
            deviceMgr.ActivateDevice(device.Result.Id, Microsoft.Azure.Devices.DeviceStatus.Enabled).Wait();

            deviceMgr.RegisterDevice("CESCO004").Wait();
            deviceMgr.ActivateDevice(device.Result.Id, Microsoft.Azure.Devices.DeviceStatus.Enabled).Wait();

            deviceMgr.RegisterDevice("CESCO005").Wait();
            deviceMgr.ActivateDevice(device.Result.Id, Microsoft.Azure.Devices.DeviceStatus.Enabled).Wait();
